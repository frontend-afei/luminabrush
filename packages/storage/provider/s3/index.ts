...
export const getSignedUploadUrl: GetSignedUploadUrlHandler = async (path, { bucket }) => {
	...
	try {
		return await getS3SignedUrl(s3Client, new PutObjectCommand({ Bucket: bucket, Key: path }), {
			expiresIn: 60,
		})
	} catch (e) {
		console.error(e)
...
export const getSignedUrl: GetSignedUrlHander = async (path, { bucket, expiresIn }) => {
	const s3Client = getS3Client()
	try {
		return getS3SignedUrl(s3Client, new GetObjectCommand({ Bucket: bucket, Key: path }), { expiresIn })
	} catch (e) {
		console.error(e)

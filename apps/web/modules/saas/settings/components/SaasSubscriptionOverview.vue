...
<script setup lang="ts">
  ...
  const activeVariantId = computed(() => {
    return props.currentSubscription
      ? props.currentSubscription.variantId
      : "free";
  });

  const subscriptionPlan = computed(() => {
    return [freePlan.value, ...props.plans].find(
      (plan) => plan.id === activePlanId.value,
    );
  });
  const subscriptionVariant = computed(() => {
    return subscriptionPlan.value?.variants.find(
      (variant) => variant.id === activeVariantId.value,
    );
  });
  ...
  const subscriptionIsExceeding = computed(() => {
    ...
    return (
      ...
      props.currentSubscription.status === "PAUSED"
    );
  });
  ...
  const nextPaymentLabel = computed(() => {
    ...
    const nextPaymentDate = currentSubscription.nextPaymentDate

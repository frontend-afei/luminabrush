<template>
	<SaasActionBlock>
		...
		<div class="flex items-center gap-4">
			...
			<div ref="dropZoneRef" class="relative cursor-pointer rounded-full" @click="openFileDialog()">
				<UserAvatar class="size-24 cursor-pointer text-xl" :avatarUrl="user?.avatarUrl" :name="user?.name ?? ''" />

				<div v-if="uploading" class="bg-card/90 absolute inset-0 z-20 flex items-center justify-center">
...
<script setup lang="ts">
	...
	const { reloadUser, currentTeam, user } = useUser()
	const { apiCaller } = useApiCaller()
	const { toast } = useToast()
	const { t } = useTranslations()

	const getSignedUploadUrlMutation = apiCaller.uploads.signedUploadUrl.useMutation()
